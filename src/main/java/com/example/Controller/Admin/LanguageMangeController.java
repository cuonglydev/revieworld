...
public class LanguageMangeController {
    ...
    public String deleteLanguage(@PathVariable("id") int id,
        ...
        System.out.println("ở ngoài"); // Log để debug
        try {
            System.out.println("xoá thành công");
            System.out.println("ID" + id);
            System.out.println("OrderTypeId" + orderTypeId);
            languageService.deleteLanguageById(id); // Xoá ngôn ngữ theo ID
            redirectAttributes.addFlashAttribute("success", "Xóa thành công"); // Gửi thông báo thành công
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("danger", "Xóa thất bại"); // Gửi thông báo lỗi
            System.out.println("Xóa thất bại");
        }

        return "redirect:/admin/order-type"; // Redirect về danh sách orderType
    }
    ...
    public String createLanguage(@ModelAttribute("langguage") Language language,
        ...
        try {
            // Kiểm tra tên ngôn ngữ đã tồn tại chưa
            ...
            // Gán OrderType cho ngôn ngữ trước khi lưu
            OrderType orderType = orderTypeService.getOrderTypeById(orderTypeId).orElse(null);
            language.setOrderType(orderType);
            languageService.saveLanguage(language); // Lưu ngôn ngữ vào DB

            // Gửi thông báo thành công
            redirectAttributes.addFlashAttribute("success",
                    "Thêm Ngôn ngữ cho" + " " + orderType.getName() + " Thành Công");
