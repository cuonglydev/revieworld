...
public class SystemController {
    ...
    public String systemPage(Model model) {
        ...
        SystemSetting settings = systemSettingService.getLatestSettings();
        if (settings == null) {
            settings = new SystemSetting();
            settings.setMinDepositAmount(5.0);
            settings.setMaxDepositAmount(10000.0);
            settings.setUsdToVndRate(23000.0);
            settings.setCommissionRate(5.0);
            settings.setAffiliateCommissionPercentage(5.0);
            settings.setReferralCommissionRate(5.0);
            settings.setCurrencyExchangeRate(23000.0);
        }
