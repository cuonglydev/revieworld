...
//    private static final Logger logger = LoggerFactory.getLogger(UserBankController.class);
    ...
    @PostMapping("/account/user-bank/save")
    public String saveUserBank(@ModelAttribute UserBank userBank, RedirectAttributes redirectAttributes, HttpServletRequest request) {
    	...
    	try {
    		...
    		if(userBank.getId() != 0) {
    			UserBank existUserBank = userBankService.findById(userBank.getId());
        		userBank.setCreatedAt(existUserBank.getCreatedAt());
        		redirectAttributes.addFlashAttribute("success", "Cập nhật thông tin rút tiền thành công!");
    		}else {
    			userBank.setCreatedAt(new Date());
    			redirectAttributes.addFlashAttribute("success", "Thêm thông tin rút tiền thành công!");
			}
    		...
    		redirectAttributes.addFlashAttribute("danger", "Thêm thông tin rút tiền thất bại!");
		}
    ...
    @GetMapping("/account/user-bank/delete/{id}")
    public String deleteUserBankFunction(@PathVariable int  id, RedirectAttributes redirectAttributes, HttpServletRequest request) {
    	try {
    		userBankService.deleteById(id);
    		redirectAttributes.addFlashAttribute("success", "Xóa thông tin rút tiền thành công!");
    	}catch (Exception e) {
			// TODO: handle exception
    		redirectAttributes.addFlashAttribute("danger", "Xóa thông tin rút tiền thất bại!");
		}
    	String referer = request.getHeader("Referer");
    	return "redirect:" + (referer != null ? referer : "/");
    }
...
//            List<UserBank> banks = userBankService.findAllByUserId(currentUser.getId());
